    ...
    return Q.nfcall(facebook, credentials)
           .then(api => {
                ...
                api.listen((err, fbmessage) => { 
                    if(!err)
                        this.handleFBNotification(fbmessage);
                });
           });
}
...
Facebot.prototype.typingLoop = function(link){
    if (! link.is_typing || ((Date.now() / 1000) - link.typing_start_time) > 60*5) //if they were typing for more than 5 minutes lets assume we missed the typing end event, also mobile devices sometimes don't stay connected to send typing end if they nav away from app
        return;
    this.ws.send(JSON.stringify({ type: 'typing', channel: link.slack_channel }));
    setInterval( ()=> this.typingLoop(link), 3000);
}
Facebot.prototype.handleTypeNotification = function(fbmessage, link){
	if (fbmessage.isTyping == link.is_typing)
		return;
...
Facebot.prototype.handleFBNotification = function(fbmessage){     
    ...
    _.where(this.channelLinks, { fb_thread: threadID.toString() })
    .forEach(link => {
            ...
            case "share":
                this.postMessage(link.slack_channel,
                                 `<${attachment.url}|${attachment.title || attachment.source || attachment.url}>: ${attachment.description || ""}`,
                                 {
                                     username: link.fb_name,
                                     icon_url: link.icon,
                                     attachments:[{image_url:attachment.image,fallback:attachment.image}]
                                 });
...
Facebot.prototype.postSlackMessagesToFB = function(message){
    var attachment = (message.type === 'message' && message.attachments && message.attachments.length > 0) ? message.attachments[0] : undefined;
    if( (this.isChatMessage(message) || attachment) &&
...
{
    var response="";
    var search_str = message.text.substring(message.text.indexOf("friends") + "friends".length).trim().toLowerCase();
    var us = this;
    Q.nfcall(this.facebookApi.getFriendsList).then(function(data){
        data.forEach( friend =>{
            if (friend.isFriend && friend.fullName.toLowerCase().indexOf(search_str) != -1){
                response += friend.fullName + " *vanity:* " + friend.vanity + " *userID:* " + friend.userID + "\n";
            }
        });
