...
// https://github.com/mishk0/slack-bot-api/pull/32
/**
 ...
 * @returns {vow.Promise}
 */
slackbots.prototype.postReactionToChannel = function(id, emoji, ts, params) {
    params = extend({
        channel: id,
        name: emoji,
        timestamp: ts
 ...
 * @returns {vow.Promise}
 */
slackbots.prototype.removeReactionFromChannel = function(id, emoji, ts, params) {
    params = extend({
        channel: id,
        name: emoji,
        timestamp: ts
 ...
 * @returns {vow.Promise}
 */
slackbots.prototype.getReactions = function(id, ts, params) {
    params = extend({
        channel: id,
        timestamp: ts
    }, params || {});
 ...
 * @returns {vow.Promise}
 */
slackbots.prototype.listReactions = function(id, ts, params) {
    params = extend({
        user: id
    }, params || {});
...
// message_db: optional, if specified used for tracking message state
var Facebot = function Constructor(settings, load_data, save_data, message_db){
...
Facebot.prototype.createFBApi = function(credentials){
    return Q.nfcall(facebook, credentials)
           .then(api => {
                ...
                this.ourFacebookId = api.getCurrentUserID();
                var opts = {
                    logLevel: "error",
                    listenEvents: true,
                };
                if (this.message_db){
                    opts.selfListen=true;
                    this.ourSenderInfo = {icon_url:`http://graph.facebook.com/${this.ourFacebookId}/picture?type=square`,username:"us"};
                    fbUtil.findFBUser(this.facebookApi,this.ourFacebookId,true)
                    .then( friend => {this.ourSenderInfo.username=friend.name; } )
                    .catch(function (error) {console.log("FIND FRIEND ERROR OF: " + error);});
                }

                api.setOptions(opts);
                api.listen((err, fbmessage) => { 
                    if(!err){
                        //wait to make sure it had a chance to get added to db first...
                        setTimeout( ()=> this.handleFBNotification(fbmessage), 250 );
                    }
                });
           });
}
...
// until we hear back they are not typing, or several minutes have past
Facebot.prototype.typingLoop = function(typingObj, link) {
    if (
        link.is_typing != typingObj || Date.now() / 1000 - link.typing_start_time > 60 * 5 //if they were typing for more than 5 minutes lets assume we missed the typing end event, also mobile devices sometimes don't stay connected to send typing end if they nav away from app
    )
        return;
    this.ws.send(JSON.stringify({ type: 'typing', channel: link.slack_channel }), err => {if (err) console.log("Error sending typing notice due to: " + err); } );
	
    setTimeout(() => this.typingLoop(typingObj, link), 3000);
};
Facebot.prototype.handleTypeNotification = function(fbmessage, link) {
    if (fbmessage.isTyping == (link.is_typing != null) ) return;
    var typingObj = link.is_typing = fbmessage.isTyping ? new Object() : null; //we use an object as something of a lock to avoid multiple typingLoops running at once
    if (typingObj != null) {
        link.typing_start_time = Date.now() / 1000;
        this.typingLoop(typingObj, link);
    }
}
Facebot.prototype.postFBMessageToSlack = function (fbmessage, link) {
    if (this.message_db)
            promise = Q.nfcall(this.message_db.message_exists,fbmessage.messageID);
        else
            promise = Q.fcall(function () {return false;});
    promise.then( already_exists => {
        if (already_exists)
            return;
        if (fbmessage.senderID != this.ourFacebookId)
        	link.is_typing=null; //while officially facebook sends not typing notifications it seems sometimes it assumes a message coming in does so, this seems to be how the actual client acts
        if (fbmessage.body !== undefined){
            var message_text = emoji.replace_emoticons_with_colons(fbmessage.body);
            var sender_info = { username: link.fb_name, icon_url: link.icon };
            if (fbmessage.senderID == this.ourFacebookId)
                sender_info = this.ourSenderInfo;
            this.postMessage(link.slack_channel,
                             message_text,
                             sender_info)
            .then( data => {
            if (this.message_db)
                this.message_db.insert_message({fb_threadid:fbmessage.threadID,fb_message_id:fbmessage.messageID,slack_message_timestamp:data.message.ts,fb_message_timestamp:fbmessage.timestamp},function(err){if (err) throw new Error("err adding of: " + err);});
        });
        }
        this.handleFBAttachments(fbmessage, link);
    });
}
Facebot.prototype.handleReadReceipt = function(fbmessage, link){
    Q.nfcall(this.message_db.get_cur_read_msg_on_thread,fbmessage.threadID)
            .then ( timestamp => {if (timestamp) this.removeReactionFromChannel(link.slack_channel,"eye",timestamp);return null;} )
            .then ( () => {return Q.nfcall(this.message_db.get_and_set_read_msg_on_thread,fbmessage.threadID,fbmessage.time);} )
            .then( timestamp => {return timestamp ? this.postReactionToChannel(link.slack_channel,"eye",timestamp) : null;} )
            .catch(function (error) {console.log("READ RECEIPT ERROR OF: " + error.error);})
            .done();
}
...
Facebot.prototype.handleFBNotification = function(fbmessage) {
    ...
    if (fbmessage.type == "message" || fbmessage.type == "read_receipt")
        threadID = fbmessage.threadID;
    if (fbmessage.type == "typ")//not sure why typing messages appear on a different thread but they do so we need to use from...
    ...
    _.where(this.channelLinks, {
        ...
        case "read_receipt":
            if (this.message_db)
                this.handleReadReceipt(fbmessage,link);
            break;
            case "message":
                ...
                (err, msgInfo) => {
                    if(err)
                        this.postMessage(link.slack_channel,
                                         ...
                                         { as_user: true });
                       else if (this.message_db)
                        this.message_db.insert_message({fb_threadid:link.fb_thread,fb_message_id:msgInfo.messageID,slack_message_timestamp:message.ts,fb_message_timestamp:msgInfo.timestamp},function(err){if (err) throw new Error("err adding of: " + err);});
