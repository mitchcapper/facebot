...
Facebot.prototype.typingLoop = function(link){
    if (! link.is_typing || ((Date.now() / 1000) - link.typing_start_time) > 60*10) //if they were typing for more than 10 minutes lets assume we missed the typing end event
        return;
    var us = this;
    this.ws.send(JSON.stringify({ type: 'typing', channel: link.slack_channel }));
    fbUtil.sleep(3000).then( ()=> us.typingLoop(link));
